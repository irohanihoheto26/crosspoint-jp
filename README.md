...
- **動的壁紙** — 今年の経過を図形で描く壁紙（水位 / 年格子 / 升目の3種類、日付に応じて毎日変化）。日付を保持できる機種（RTC搭載＋RTC有効）でのみ利用可能
...
- **読書進捗の表示** — ホーム画面と最近読んだ本リストに進捗率を表示。ファイル一覧では未読/読書中/読了をアイコンで識別可能（95%以上読了で自動判定）
- **UIテーマ「Vega」** — 角丸カードとタイル状メニューのホーム画面（設定から Lyra / Lyra 拡張 と切り替え可）
